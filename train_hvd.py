...
Distributed data training with Horovod added by Wushi Dong, Samuel Flender, and Murat Keceli.
...
flags.DEFINE_list('reflectable_axes', ['0', '1', '2'],
                  ...
                  'in order to augment the training data.')
#New flags
flags.DEFINE_integer('sharding_rule',0,'0: do not shard; 1: shard data')
flags.DEFINE_integer('warmup_steps', 0, 'Number of warmup steps.')
flags.DEFINE_integer('save_steps', 100, 'Number of steps for saving.')
...
def get_learning_rate(step,batch_size=1):
    ...
    if step > FLAGS.warmup_steps:
      if FLAGS.scaling_rule == 1:
        target_lr = target_lr * hvd.size()
      elif FLAGS.scaling_rule == 2:
        target_lr =  target_lr * np.sqrt(hvd.size())
      elif FLAGS.scaling_rule == 3:
        target_lr =  target_lr * hvd.size() * batch_size
      elif FLAGS.scaling_rule == 4:
        target_lr =  target_lr * np.sqrt(hvd.size() * batch_size)
      else:
        pass
    return target_lr
